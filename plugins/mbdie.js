...
const mobs = {
    ...
    "entity.zombie_pigman.name": "僵尸zhul",
    "entity.zombie_villager.name": "僵尸村民",
    "entity.zombie_villager_v2.name": "怪人村民"
}

function on_mob_die(ser,pack){
    var p = JSON.parse(pack);
    if(p.cause == 'mobdie'){
        var mob = "entity." + p.params.srctype.toLowerCase() + ".name";
        if(mobs[mob] != undefined){
            NIL.bot.sendChatMessage(NIL.LANG.get('MEMBER_KILL_BY_MOBS',ser,p.params.mobname,mobs[mob]));
        }
    }
}

function onStart(){
    NIL.FUNC.PLUGINS.WS.push(on_mob_die);
    NIL.Logger.info('mobdie','插件装载成功');
}

function onStop(){
    NIL.Logger.info('mobdie','插件已卸载');
}
